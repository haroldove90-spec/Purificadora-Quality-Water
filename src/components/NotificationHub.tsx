...
export default function NotificationHub({ userRole }: NotificationHubProps) {
  const { notifications, markAsRead, clearAll } = useRealtimeNotifications(userRole);
  ...
  return (
    <div className="relative">
      <button 
        ...
        className="relative p-2 rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors text-slate-600"
      >
        <Bell size={20} />
        {unreadCount > 0 && (
            ...
            >
              <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Tiempo Real • Supabase Live</p>
                </div>
                <button 
                  onClick={clearAll}
              ...
              <div className="max-h-[450px] overflow-y-auto no-scrollbar py-2">
                {notifications.length === 0 ? (
                  <div className="p-12 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-400 italic">No tienes nuevas notificaciones</p>
                  </div>
                ) : (
              ...
              <div className="p-4 border-t border-slate-50 bg-slate-50/50">
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest text-center">
                  Monitoreando Canal: matrix_orders_v1
                </p>
              </div>
            </motion.div>
          </>
