...
export default function SalesHistoryEmployees({ userRole }: { userRole: string }) {
  ...
  // Filtered employee list based on search bar
  ...
  // Filter orders based on assigned employee, dates, status, and channel
  const getFilteredOrders = () => {
    return orders.filter(o => {
      // Filter by employee
      if (selectedEmployeeName) {
        const assignedName = o.assigned_to_name || '';
        // Handle "Mostrador/Planta" fallback
        ...
        // Smart Search query in global employee view
        if (searchQuery) {
          const q = searchQuery.toLowerCase().trim();
  ...
  const calculateStats = () => {
    ...
    return { totalPurchases, totalAmount, avgTicket, deliveredCount, pendingCount, successRate, favoriteItem };
  };
  ...
  const handleExportPDF = () => {
    ...
    const subtitleText = selectedEmployeeName
      ? `Periodo: ${startDate} al ${endDate} | Total Recaudado: $${stats.totalAmount.toFixed(2)} pesos`
      : `Periodo: ${startDate} al ${endDate} | Ventas Totales: $${stats.totalAmount.toFixed(2)} pesos`;

    const columns = ['Fecha/Hora', 'Atendió/Entregó', 'Cliente', 'Folio', 'Artículos', 'Canal/Origen', 'Estado', 'Total'];
  ...
  const handleExportExcel = () => {
    const headers = ['Fecha/Hora', 'Empleado Atendio', 'Cliente', 'Folio Pedido', 'Articulos', 'Canal de Venta', 'Estado', 'Monto Total'];
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-400 max-w-xl font-medium leading-relaxed">
              Supervise y evalúe la facturación de cada chofer, repartidor o personal de planta. Obtenga indicadores de rendimiento de entrega y exporte informes.
            </p>
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-white rounded-[32px] border border-slate-100 p-6 shadow-sm flex flex-col h-[750px]">
          ...
          >
            ...
            <div className="space-y-0.5">
              ...
              <p className={`text-[9px] font-bold uppercase tracking-wider ${selectedEmployeeName === null ? 'text-teal-300' : 'text-slate-400'}`}>
                Comparativa de toda la planilla
              </p>
            </div>
          </button>
          ...
          <div className="space-y-4 mb-4">
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
              <Search size={14} className="text-slate-400" /> Directorio de Empleados
            </h3>
            <div className="relative">
              <input 
                type="text"
                placeholder={selectedEmployeeName === null ? "Buscar en todo el historial..." : "Buscar colaborador..."}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl font-bold focus:ring-2 focus:ring-emerald-500 outline-none text-slate-700 placeholder-slate-400"
              />
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-slate-400 py-12 text-center space-y-2">
                ...
                <p className="text-[11px] text-slate-400 font-medium px-4">No se encontraron colaboradores.</p>
              </div>
            ) : (
              <>
                ...
                >
                  <div className="space-y-1 flex-1 min-w-0 pr-2">
                    <p className={`font-black text-[13px] uppercase truncate ${selectedEmployeeName === 'Planta' ? 'text-emerald-950 font-black' : 'text-slate-700'}`}>
                      🏪 Planta / Mostrador (Venta Directa)
                    </p>
                    <p className="text-[10px] text-emerald-600 font-bold">Oficina Principal</p>
                  </div>
                ...
                {activeEmployeeList.map((emp) => {
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-1 flex-1 min-w-0 pr-2">
                        <p className={`font-black text-[13px] uppercase truncate ${isSelected ? 'text-emerald-950' : 'text-slate-700 group-hover:text-slate-900'}`}>
                          {emp.name}
                        </p>
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 p-6 shadow-sm space-y-6">
            ...
            {/* Date and rapid filtering controls */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-white p-3 border-none rounded-xl font-bold text-xs focus:ring-2 focus:ring-emerald-500 outline-none text-slate-600"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-white p-3 border-none rounded-xl font-bold text-[10px] focus:ring-2 focus:ring-emerald-500 outline-none text-slate-600 appearance-none uppercase"
                >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                <span className="text-[8px] font-bold text-emerald-600 uppercase mt-0.5 block">Delivered Rate</span>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden flex-1 min-h-[300px]">
            <div className="p-6 border-b border-slate-100">
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">Haga uso de los filtros superiores para auditar el desempeño de su personal</p>
            </div>
            ...
            {selectedOrders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-slate-400 italic text-center space-y-2">
                ...
                <p className="text-[10px] text-slate-400 font-bold px-6">No existen pedidos asociados a este colaborador o bajo los filtros aplicados.</p>
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs text-slate-600 font-semibold uppercase">
                    {selectedOrders.map((o) => (
                      <tr key={o.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-[10px] font-mono text-emerald-600 font-bold">
                          #{o.id.slice(0, 8).toUpperCase()}
                        </td>
